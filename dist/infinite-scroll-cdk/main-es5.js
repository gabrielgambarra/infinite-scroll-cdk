...
var FactScrollerComponent = /** @class */ (function () {
    function FactScrollerComponent(factService) {
        this.factService = factService;
        this.dataSource = new FactsDataSource(factService);
    }
    FactScrollerComponent.ctorParameters = function () { return [
    ...
    return FactScrollerComponent;
}());

var FactsDataSource = /** @class */ (function (_super) {
    tslib__WEBPACK_IMPORTED_MODULE_0__["__extends"](FactsDataSource, _super);
    function FactsDataSource(factService) {
        var _this = _super.call(this) || this;
        _this.factService = factService;
        _this.cachedFacts = Array.from({ length: 0 });
        _this.dataStream = new rxjs__WEBPACK_IMPORTED_MODULE_3__["BehaviorSubject"](_this.cachedFacts);
        _this.subscription = new rxjs__WEBPACK_IMPORTED_MODULE_3__["Subscription"]();
        _this.pageSize = 10;
        _this.lastPage = 0;
        // Start with some data.
        _this._fetchFactPage();
        return _this;
    }
    FactsDataSource.prototype.connect = function (collectionViewer) {
        var _this = this;
        this.subscription.add(collectionViewer.viewChange.subscribe(function (range) {
            var currentPage = _this._getPageForIndex(range.end);
            if (currentPage && range) {
                console.log(currentPage, _this.lastPage);
            }
            if (currentPage > _this.lastPage) {
                _this.lastPage = currentPage;
                _this._fetchFactPage();
            }
        }));
        return this.dataStream;
    };
    FactsDataSource.prototype.disconnect = function (collectionViewer) {
        this.subscription.unsubscribe();
    };
    FactsDataSource.prototype._fetchFactPage = function () {
        var _this = this;
        for (var i = 0; i < this.pageSize; ++i) {
            this.factService.getRandomFact().subscribe(function (res) {
                _this.cachedFacts = _this.cachedFacts.concat(res);
                _this.dataStream.next(_this.cachedFacts);
            });
        }
    };
    FactsDataSource.prototype._getPageForIndex = function (i) {
        return Math.floor(i / this.pageSize);
    };
    FactsDataSource.ctorParameters = function () { return [
        { type: _services_fact_service__WEBPACK_IMPORTED_MODULE_4__["FactService"] }
    ]; };
...
var FactService = /** @class */ (function () {
    ...
    FactService.prototype.getRandomFact = function () {
        ...
        return this.http.get("https://cors-anywhere.herokuapp.com/http://numbersapi.com/" + month + "/" + day + "/date?json");
    };
